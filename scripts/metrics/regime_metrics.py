...
class RegimeAnalyzer:
    ...
    def __init__(self, sma_period: int = 200, volatility_window: int = 30):
        ...
        logger.info(f"RegimeAnalyzer initialized with SMA: {sma_period}, Volatility window: {volatility_window}")
    
    def detect_regimes(self, price_data: pd.Series, 
                      volatility_data: Optional[pd.Series] = None) -> pd.DataFrame:
        ...
        try:
            ...
            regimes['price_regime'] = np.where(
                price_data > sma, 'BULL', 'BEAR'
            )
            ...
            if volatility_data is not None:
                ...
                rolling_vol = daily_returns.rolling(window=self.volatility_window).std() * np.sqrt(252)
            ...
            logger.info(f"Detected regimes: {regimes['combined_regime'].value_counts().to_dict()}")
    ...
    def calculate_regime_metrics(self, returns: pd.Series, 
        ...
        try:
            ...
            aligned_data = pd.DataFrame({
                'returns': returns,
                'combined_regime': regimes['combined_regime']
            }).dropna()
            ...
            for regime in aligned_data['combined_regime'].unique():
                regime_returns = aligned_data[aligned_data['combined_regime'] == regime]['returns']
                ...
                regime_metrics[f'{regime_key}_annual_return'] = regime_returns.mean() * 252
                regime_metrics[f'{regime_key}_volatility'] = regime_returns.std() * np.sqrt(252)
                
                # Risk-adjusted metrics
                if regime_returns.std() != 0:
                    regime_metrics[f'{regime_key}_sharpe'] = (
                        regime_returns.mean() * 252 / (regime_returns.std() * np.sqrt(252))
                    )
                else:
    ...
    def get_regime_performance_summary(self, returns: pd.Series, 
                                     regimes: pd.DataFrame) -> str:
        ...
        try:
            ...
            # Get unique regimes
            regime_keys = [k for k in regime_metrics.keys() if '_return' in k and not 'annual_' in k]
            ...
            for key in regime_keys:
                ...
                win_rate = regime_metrics.get(f'{key.replace("_return", "")}_win_rate', 0)
