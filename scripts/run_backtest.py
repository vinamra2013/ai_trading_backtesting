...
class BacktestRunner:
    ...
    def _prepare_artifacts(self, backtest_results: Dict, output_dir: Path) -> Dict:
        ...
        try:
            ...
            equity_csv_path = output_dir / f"{backtest_results['backtest_id']}_equity_curve.csv"
            equity_curve_df.to_csv(equity_csv_path, index=False)
            artifacts['equity_curve'] = str(equity_csv_path)
            
            # Trade log artifact
            trades_df = pd.DataFrame(backtest_results['trades'])
            trades_csv_path = output_dir / f"{backtest_results['backtest_id']}_trades.csv"
            trades_df.to_csv(trades_csv_path, index=False)
            artifacts['trade_log'] = str(trades_csv_path)
            
            # Backtest summary artifact
            summary_path = output_dir / f"{backtest_results['backtest_id']}_summary.json"
    ...
    def run(self, strategy_path: str, symbols: List[str], start_date: str,
        ...
        if mlflow_enabled and self.mlflow_enabled:
            experiment_name = self._build_experiment_name(project, asset_class, 
                                                        strategy_family, strategy_class.__name__)
            tags = self._build_tags(project, asset_class, strategy_family, 
                                  strategy_class.__name__, team, status,
                                  symbols=str(symbols),
        ...
        if ADVANCED_METRICS_AVAILABLE:
                ...
                if len(equity_df) > 0:
                    ...
                    if len(returns_series) > 0:
                        ...
                        try:
                            ...
                            quantstats_metrics = quantstats_analyzer.calculate_metrics(returns_series)
                        ...
                        try:
                            ...
                            regime_metrics = regime_analyzer.calculate_regime_metrics(returns_series, regimes)
                            backtest_results['regime_analysis'] = {
                                'regime_breakdown': regime_metrics,
                                'regime_summary': regime_analyzer.get_regime_performance_summary(returns_series, regimes)
                            }
                        ...
                        if metrics_calculated > 0:
                            print(f"   ✅ Advanced metrics calculated: {metrics_calculated} total metrics")
                        else:
                            print("   ⚠️  No advanced metrics could be calculated")
                    else:
                        print("   ⚠️  Insufficient returns data for advanced metrics calculation")
                        backtest_results['quantstats_metrics'] = {}
                        backtest_results['regime_analysis'] = {}
                        backtest_results['alpha_beta_metrics'] = {}
                else:
            ...
            print("   ℹ️  Advanced metrics libraries not available - skipping enhanced analysis")
            backtest_results['quantstats_metrics'] = {}
            backtest_results['regime_analysis'] = {}
            backtest_results['alpha_beta_metrics'] = {}
        ...
        if mlflow_enabled and self.mlflow_enabled:
            try:
                ...
                if 'quantstats_metrics' in backtest_results and backtest_results['quantstats_metrics']:
                    mlflow_metrics.update(backtest_results['quantstats_metrics'])
                if 'regime_analysis' in backtest_results and backtest_results['regime_analysis'].get('regime_breakdown'):
                    mlflow_metrics.update(backtest_results['regime_analysis']['regime_breakdown'])
                if 'alpha_beta_metrics' in backtest_results and backtest_results['alpha_beta_metrics']:
...
def main():
    parser = argparse.ArgumentParser(description='Run Backtrader backtest with optional MLflow integration')
    ...
    parser.add_argument('--project', default='Default', help='Project name for MLflow organization')
    parser.add_argument('--asset-class', default='Equities', help='Asset class for MLflow tagging')
    ...
    try:
        runner.run(
            strategy_path=args.strategy, symbols=args.symbols,
