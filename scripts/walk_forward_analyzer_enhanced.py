...
class EnhancedWalkForwardAnalyzer:
    ...
    def __init__(self, mlflow_tracking_uri: str = "http://172.25.0.4:5000"):
        """
    ...
    def run_walk_forward_analysis(
        ...
        """
        logger.info("Starting enhanced walk-forward analysis for strategy: %s", strategy_path)
        ...
        logger.info("Enhanced walk-forward analysis completed. Average out-of-sample Sharpe: %.4f",
    ...
    def _generate_analysis_windows(
        ...
        while current_date < datetime.strptime(end_date, '%Y-%m-%d'):
            ...
            window_end = window_start + timedelta(days=window_size_months*30)  # Approximate

            # Split into train/test (80/20 split)
            train_end = window_start + timedelta(days=int(window_size_months*30*0.8))
    ...
    def _analyze_window_parallel(
        ...
        try:
            ...
            if not results_df.empty:
                metrics = {
                    'total_return': results_df['total_return'].mean() if 'total_return' in results_df.columns else 0.0,
                    'sharpe_ratio': results_df['sharpe_ratio'].mean() if 'sharpe_ratio' in results_df.columns else 0.0,
                    'max_drawdown': results_df['max_drawdown'].max() if 'max_drawdown' in results_df.columns else 0.0,
                    'total_trades': results_df['trade_count'].sum() if 'trade_count' in results_df.columns else 0,
    ...
    def _analyze_walk_forward_results(
        ...
        out_of_sample_sharpes = [r['out_of_sample_metrics'].get('sharpe_ratio', 0)
                                for r in successful_windows]
        out_of_sample_returns = [r['out_of_sample_metrics'].get('total_return', 0)
                                for r in successful_windows]
        ...
        return {
            'summary': summary,
            'window_results': window_results,
            'performance_stability': self._calculate_stability_metrics(out_of_sample_sharpes)
        }

    def _calculate_stability_metrics(self, sharpe_ratios: List[float]) -> Dict[str, Any]:
        ...
        positive_sharpe_ratio = sum(1 for s in sharpe_ratios if s > 0) / len(sharpe_ratios)

        # Calculate stability (inverse of coefficient of variation)
        mean_sharpe = np.mean(sharpe_ratios)
        std_sharpe = np.std(sharpe_ratios)
        stability_score = mean_sharpe / (std_sharpe + 0.1)  # Add small epsilon to avoid division by zero
    ...
    def _log_walk_forward_to_mlflow(
        ...
        try:
            ...
            # Log metrics
            metrics = {
                'avg_out_of_sample_sharpe': results['summary']['avg_out_of_sample_sharpe'],
                'avg_out_of_sample_return': results['summary']['avg_out_of_sample_return'],
                ...
                'consistency_ratio': results['performance_stability']['consistency_ratio']
            }

            # Create experiment name
            experiment_name = f"{project}.{asset_class}.{strategy_family}.{strategy_name}"

            # Log to MLflow (simplified - would need to extend MLflowBacktestLogger)
            logger.info("Walk-forward analysis logged to MLflow experiment: %s", experiment_name)
            logger.info("Results: Avg Sharpe=%.3f, Stability=%.3f, Consistency=%.1f%%",
...
Examples:
  ...
  python scripts/walk_forward_analyzer_enhanced.py \\
    ...
    parser.add_argument('--step-size', type=int, default=3,
                       help='Step size in months (default: 3)')
    parser.add_argument('--optimization-metric', default='sharpe_ratio',
                       help='Metric to optimize (default: sharpe_ratio)')
    parser.add_argument('--project', default='walk_forward',
                       help='MLflow project name')
    parser.add_argument('--asset-class', default='equities',
                       help='Asset class for tagging')
    parser.add_argument('--strategy-family', default='unknown',
                       help='Strategy family for tagging')
    ...
    try:
        ...
        print(f"Windows: {summary['successful_windows']}/{summary['total_windows']} successful")
        print(f"Average Out-of-Sample Sharpe: {summary['avg_out_of_sample_sharpe']:.3f}")
        print(f"Average Out-of-Sample Return: {summary['avg_out_of_sample_return']:.3f}")
