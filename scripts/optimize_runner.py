...
def parse_args():
    ...
    parser.add_argument(
        '--batch',
        type=str,
        help='Path to batch config file (multiple optimizations)'
    )
    parser.add_argument(
        '--estimate',
        action='store_true',
        help='Estimate fee impact only (dry run)'
    )
    parser.add_argument(
        '--no-import',
        action='store_true',
        help='Skip results import to database'
    )
...
def calculate_combinations(parameters):
    ...
    for param_name, param_config in parameters.items():
        ...
        # Calculate number of values for this parameter
...
def estimate_fees(config):
    ...
    print(f"   Estimated time (@ 30 sec/backtest): {(total_combinations * 30) / 60:.1f} minutes")
...
def build_lean_optimize_command(config):
    ...
    cmd = [
        ...
        '--target', opt_config['target_metric'],
        '--target-direction', opt_config['target_direction'],
...
def wait_for_optimization_completion(project_name, optimization_id=None, poll_interval=30, max_wait_minutes=120):
...
def run_optimization(config):
    ...
    try:
        ...
        container_match = re.search(r"in the '([^']+)' container", result.stdout)
        ...
        if container_match:
            ...
            results_dir = None
            if output_match:
                # Build full path to results directory
                results_dir = LEAN_PROJECTS_DIR / output_dir

            stall_count = 0
            stall_threshold = 6  # 6 checks with no results = 60 seconds
            ...
            while True:
                ...
                # Check if result files exist (best indicator of completion)
                if results_dir and results_dir.exists():
                    # LEAN creates subdirectories for each backtest run
                    # Look for JSON files in subdirectories (excluding config files)
                    json_files = [f for f in results_dir.glob('**/*.json')
                                  if f.name not in ['optimizer-config.json', 'config.json']
                                  and 'summary.json' in f.name]  # Only count result files, not logs

                    if json_files:
                        print(f'\n\n✅ Found {len(json_files)} result files - optimization complete!')
                        print('   Force-killing container (results written)...')
                        subprocess.run(['docker', 'kill', container_name], check=False)
                        time.sleep(2)  # Give Docker time to clean up
                        return True
                    else:
                        # Directory exists but no result files yet
                        stall_count = 0  # Reset stall counter
                else:
                    # Results directory doesn't exist yet, check for stall
                    stall_count += 1
                    if stall_count >= stall_threshold:
                        print(f'\n\n⚠️  No results after {stall_threshold * 10}s')
                        ...
                        if 'Error' in logs_result.stdout or 'Exception' in logs_result.stdout:
                            print('   ❌ Errors detected in logs - optimization may have failed')
                            return False
...
def import_results(config):
    ...
    optimization_runs = sorted(optimizations_dir.iterdir(), key=lambda x: x.stat().st_mtime, reverse=True)
...
def print_summary(config):
    ...
    try:
        ...
        if result and result[0] > 0:
            ...
            print("SUMMARY")
            print("="*60)
            print(f"{total_runs} runs complete, {passed or 0} passed, best Sharpe: {best_sharpe or 0:.2f}")
            print("="*60)
        else:
...
def main():
    ...
    if not os.path.exists('/.dockerenv'):
        ...
        try:
            ...
            result = subprocess.run(
                ['docker', 'ps', '-a', '--filter', 'name=lean-optimizer', '--format', '{{.Names}}'],
                capture_output=True,
                text=True,
                check=True
            )
            ...
            if 'lean-optimizer' in result.stdout:
                ...
                subprocess.run(['docker', 'compose', 'up', '-d', 'lean-optimizer'],
                             cwd=PROJECT_ROOT, check=False)

                # Build docker exec command
                # Use absolute host paths since container mounts at same path
                docker_cmd = [
                    'docker', 'exec', '-it', 'lean-optimizer',
                    'python', str(PROJECT_ROOT / 'scripts' / 'optimize_runner.py')
                ]
                ...
                print("   Run 'docker compose up -d lean-optimizer' to enable Docker mode")
    ...
    if args.batch:
        print("❌ Batch mode not yet implemented. Use --config for single optimization.")
        return 1
